...
This file provides context about this project for AI assistants like in Cursor.
...
- **Current tools:**
  ...
  - `add_numbers`: Adds two numbers together (example of a tool with parameters)
  - `trigger_job_run`: Triggers a Databricks job run by job ID (uses app-level authentication)
...
**Note:** The `get_current_user` and `trigger_job_run` tools are already implemented in `server/tools.py` and serve as real examples.
...
3. **Remote Deployment with OAuth** (Interactive): 
   ...
   - `scripts/dev/query_remote.py` - Tests all tools including health, user auth, add_numbers, and optionally trigger_job_run
...
5. Optionally prompts for a job ID to test `trigger_job_run` tool
6. Generates OAuth token using `generate_oauth_token.py` with correct scopes
7. Tests MCP client with user authentication via `query_remote.py`
8. Calls all available tools: `health`, `get_current_user`, `add_numbers`, and optionally `trigger_job_run`
