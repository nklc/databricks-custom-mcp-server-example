...
# Step 4.5: Optionally get job ID for testing trigger_job_run
echo "Optional: Enter a Databricks job ID to test trigger_job_run tool"
read -p "(Press Enter to skip): " test_job_id

if [ -n "$test_job_id" ]; then
    echo "✓ Will test trigger_job_run with job ID: $test_job_id"
else
    echo "✓ Skipping trigger_job_run test"
fi
echo ""
...
# Build command with optional job ID parameter
cmd="uv run python $SCRIPT_DIR/query_remote.py --host $host --token $token --app-url $app_url"
if [ -n "$test_job_id" ]; then
    cmd="$cmd --test-job-id $test_job_id"
fi
